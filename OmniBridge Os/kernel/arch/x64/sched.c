...
void schedule(void)
{
    ...
    if (next) {
         ...
         *
         * 人工必须审查：
         *   TSS.rsp0 决定从 CPL=3 进入 CPL=0（中断/异常）时 CPU 使用
         *   哪个内核栈。之前它只在 main.c 的 tss_init() 中被设置为
         *   kernel_stack_top（全局静态内核栈），从不更新。
         *
         *   后果：
         *     - 任意用户态线程被 LAPIC/PIT 中断时，CPU 都压 iretq
         *       帧到同一个 kernel_stack_top；
         *     - 若同时有多个用户态线程（如 pthread 测试）在运行，
         *       它们的 iretq 帧互相覆盖，导致 iretq 恢复错误的 RIP
         *       （症状：RIP=0xb0001 之类的垃圾值，#UD panic）；
         *     - 更危险的是，用户态线程 A 被中断进入内核后被抢占，
         *       用户态线程 B 再被中断，会覆盖 A 的 iretq 帧。
         *
         *   修复：每次 schedule() 切换到 next 之前，把 TSS.rsp0 更新
         *   为 next 的 kstack 栈顶。这样无论 next 何时从用户态被中断，
         *   都使用它自己的 kstack，不会互相污染。
         *
         *   next 是内核线程（kstack_base 非 0）时同样更新——虽然内核
         *   线程不会 iretq 到用户态，但保持 TSS.rsp0 = 当前运行线程的
         *   kstack top 是语义上最清晰的做法。
         */
