 ...
 */
/*
 * ★ 第 19 步：clone 语义升级 —— 支持真 fork
 *
 * 人工必须审查：
 *   - 若 flags 含 CLONE_VM，则共享父进程 user_ctx（现有 18D 行为）。
 *   - 若不含 CLONE_VM，则调用 user_fork_address_space 复制父进程用户
 *     地址空间（PML4[1..255] 中用户映射 + 每页 deep copy）。
 *   - user_fork_address_space 内部执行完整回滚；本函数只需处理
 *     命名空间与 task 创建失败的回滚。
 */
int64_t namespace_clone_child(struct task_t *cur, uint64_t flags)
{
    ...
    extern int user_fork_address_space(struct task_t *parent,
                                        struct task_t *child);
    ...
    if (flags & CLONE_VM) {
        /* 共享父进程地址空间：直接沿用 user_ctx 指针（后续可升级为
         * 引用计数） */
        if (cur->user_ctx) {
        ...
        /* 真 fork：复制父进程用户地址空间。
         * user_fork_address_space 会为 child 分配新 user_ctx。 */
        if (cur->user_ctx) {
            rc = user_fork_address_space(cur, child);
            if (rc != 0) {
                extern void task_destroy(struct task_t *t);
                task_destroy(child);
                return rc;
