...
int64_t syscall_dispatcher(struct syscall_frame *f)
{
     ...
     *
     * 人工必须审查：
     *   - COMPAT_TYPE_LINUX 的进程，其 rax 是 Linux syscall 号（0..~450），
     *     与 OB 系统调用号（0x100..0x15F）没有交集。因此在进入 OB
     *     分发前必须先判断 compat_type。
     *   - 该分流必须先于 sandbox/see 检查，因为兼容层自身也是原生
     *     权限检查的子集；沙盒检查在 Linux handler 内部由
     *     check_permission 覆盖。 */
    if (cur->compat_type == COMPAT_TYPE_LINUX) {
        extern int64_t linux_syscall_dispatch(struct task_t *cur,
                                              struct syscall_frame *f);
        int64_t r = linux_syscall_dispatch(cur, f);
        return r;
    }

        /* ★ 第 20 步：Windows PE 兼容层分流。
     *
     * 人工必须审查：
     *   - Win32 API 编号从 0x2001 起，与 OB syscall (0x100..0x15E)
     *     和 Linux syscall (0..~450) 无交集。
     *   - 所有 Win32 API 内部自行做 check_permission。
     */
    if (cur->compat_type == COMPAT_TYPE_WIN32) {
        extern int64_t win32_api_dispatch(struct task_t *cur,
                                          struct syscall_frame *f);
        int64_t r = win32_api_dispatch(cur, f);
        return r;
    }
