...
/* ============================================================
 * ★ 第 20 步 v3 关键修复：syscall 内核栈顶的 per-thread 更新
 *
 * 人工必须审查：
 *   g_syscall_kernel_rsp 是**全局**变量。syscall_entry 进入时读取它，
 *   切换到该地址作为内核栈。它必须反映"当前将要进入用户态的线程"
 *   的内核栈顶。任何一次 iretq 进入用户态之前都必须更新它，否则：
 *
 *   - 用户态 syscall 会使用别的线程的内核栈；
 *   - 同进程内多个 pthread 线程的 syscall 帧互相覆盖；
 *   - syscall 返回时读到垃圾 RIP，触发 #UD/#GP。
 *
 *   典型症状：RIP=0xb0001, CS=0x8, ERR=0, #UD。
 *
 *   正确的调用点：
 *     1) user_enter()         —— 主线程首次进入用户态前；
 *     2) user_thread_entry()  —— pthread 线程首次进入用户态前；
 *     3) user_resume_ctx()    —— 异常/sigreturn 恢复用户态前。
 * ============================================================ */
static void user_update_syscall_kstack(void)
{
    extern uint64_t g_syscall_kernel_rsp;
    ...
    if (th && th->kstack_base) {
        ...
        /* ★ 修复：同步 TSS.rsp0，确保从用户态中断进入内核时使用
         * 本线程的 kstack。详见 sched.c 中 schedule() 的注释。 */
        tss_set_rsp0(ktop & ~0xFULL);
    } else {
        g_syscall_kernel_rsp = 0;
    }
...
/* ★ 第 19 步修改：argc < 0 表示“栈已由 linux_build_stack 构建，
 * user_enter 不再触碰栈内容”。这使 Linux ELF 路径可复用整个
 * iretq 流程（fsbase / CR3 切换 / iretq）而不重复实现。 */
void user_enter(uint64_t entry, uint64_t user_rsp, int argc, const char **argv)
{
    ...
    {
        extern uint64_t g_syscall_kernel_rsp;
        user_update_syscall_kstack();
        serial_printf("[ENTER-F] set g_syscall_kernel_rsp=0x%llx\n",
                      (unsigned long long)g_syscall_kernel_rsp);
    }

    if (argc < 0) {
        /* ★ 第 19 步：调用方已构建栈，直接使用 user_rsp。 */
        serial_printf("[ENTER-G] user stack prebuilt at 0x%llx\n",
    ...
    vmm_flush_tlb();      /* ★ 新增：强制 TLB flush */
    serial_puts("[ENTER-J] CR3 switched\n");

        /* ★ 第 20 步：Win32 兼容进程需要 GS_BASE = TEB_VA，RDX = PEB VA，
     * RCX = 0。其它 compat_type 保持原有行为。 */
    if (t->compat_type == COMPAT_TYPE_WIN32) {
        extern uint64_t win32_peb_va(struct task_t *t);
        extern uint64_t win32_teb_va(struct task_t *t);
        uint64_t peb = win32_peb_va(t);
        user_set_gsbase(win32_teb_va(t));
...
static void user_test_child_entry(void *arg)
{
    ...
    int kind = 0;   /* 0=OBR, 1=ELF, 2=EXE */
    if (path) {
        const char *s = path;
        while (*s) ++s;
        if (s - path >= 4 && s[-4] == '.' && s[-3] == 'e' &&
            s[-2] == 'l' && s[-1] == 'f') kind = 1;
        else if (s - path >= 4 && s[-4] == '.' && s[-3] == 'e' &&
                 s[-2] == 'x' && s[-1] == 'e') kind = 2;
    }

    extern int user_spawn_linux_elf(const char *path);
    extern int user_spawn_win32_pe(const char *path);

    int rc;
    if (kind == 1)      rc = user_spawn_linux_elf(path);
    else if (kind == 2) rc = user_spawn_win32_pe(path);
    else                rc = user_spawn_program(path);
...
static void user_thread_entry(void *arg)
{
    ...
    /* ★ 关键修复：设置 syscall 内核栈顶，与 user_enter 保持一致。
     *
     * 人工必须审查：
     *   g_syscall_kernel_rsp 是**全局**变量，per-thread 独立，必须
     *   在本线程 iretq 进入用户态之前，指向本线程的内核栈顶。
     *
     *   原实现在 user_thread_entry 中遗漏了这一句，导致所有 pthread
     *   线程的 syscall 都使用父线程的内核栈顶——多个线程的 syscall
     *   帧互相覆盖，破坏返回路径（症状：RIP=0xb0001 之类的垃圾值，
     *   CS=8，#UD 或 #GP）。
     */
    {
        extern uint64_t g_syscall_kernel_rsp;
 ...
 *
 * 人工必须审查：
 *   1) compat_type 必须在 user_setup 之前设为 COMPAT_TYPE_LINUX，
 *      使 syscall_dispatcher 能在入口分流。
 *   2) 使用 linux_build_stack 构建 Linux 风格栈，再用
 *      user_enter(entry, rsp, argc=-1, NULL) 进入用户态。
 *   3) 全部失败路径必须 pmm_free_pages(img_pg, order) 释放镜像缓冲。
 * ============================================================ */
int user_spawn_linux_elf(const char *path)
{
    ...
    if (!t || !th) return -1;
    if (t->user_ctx) {
        serial_printf("[LINUX] spawn: pid=%llu already has user_ctx\n",
                      (unsigned long long)t->pid);
        return -1;
    }
    ...
    int rc = vfs_lookup(path, &ino);
    if (rc != 0 || !ino) {
        serial_printf("[LINUX] spawn: lookup '%s' failed rc=%d\n", path, rc);
        return -1;
    }
    uint64_t fsize = ino->size;
    if (fsize == 0 || fsize > USER_IMAGE_MAX) return -1;
    ...
    int64_t total = vfs_read(f, img, fsize);
    vfs_close(f);
    if (total != (int64_t)fsize) { pmm_free_pages(img_pg, order); return -1; }

    /* 2) 检测 ELF */
    int d = elf64_detect(img, fsize);
    if (d != 1) {
        serial_printf("[LINUX] spawn: '%s' not ELF64 (d=%d)\n", path, d);
        pmm_free_pages(img_pg, order);
        return -1;
    }
    const struct elf64_ehdr *eh = (const struct elf64_ehdr *)img;
    serial_printf("[LINUX] spawn: detected ELF64 executable, entry=0x%llx\n",
                  (unsigned long long)eh->e_entry);

    /* 3) 建立用户环境：compat_type 必须在 user_setup 之前设置 */
    t->compat_type = COMPAT_TYPE_LINUX;
    int urc = user_setup(t, eh->e_entry);
    if (urc != 0) { pmm_free_pages(img_pg, order); return urc; }

    struct user_ctx *c = user_get_ctx(t);
    if (!c) { pmm_free_pages(img_pg, order); return -1; }

    /* 4) 加载 ELF 段 */
    uint64_t entry_va = 0, phdr_va = 0, phnum = 0;
    rc = elf64_load_into_task(t, img, fsize, &entry_va, &phdr_va, &phnum);
    if (rc != 0) {
        serial_printf("[LINUX] spawn: elf64_load rc=%d\n", rc);
        pmm_free_pages(img_pg, order);
        return rc;
    }

    /* 5) 构建 Linux 栈（argc/argv/envp 本步简化为单个 argv[0]=path） */
    const char *argv0 = path;
    const char *argv_l[2] = { argv0, 0 };
    serial_printf("[LINUX] spawn: building Linux stack: argc=1 envp=0\n");
    uint64_t rsp = linux_build_stack(t, 1, argv_l, 0, 0,
                                     entry_va, phdr_va,
                                     (uint16_t)phnum, path);
    if (rsp == 0) {
        serial_printf("[LINUX] spawn: linux_build_stack failed\n");
        pmm_free_pages(img_pg, order);
        return -1;
    }
    serial_printf("[LINUX] spawn: auxv: AT_PAGESZ=0x1000 AT_ENTRY=0x%llx "
                  "AT_PHNUM=%llu\n",
                  (unsigned long long)entry_va,
    ...
    /* 7) 进入用户态（argc<0 表示栈已构建） */
    serial_printf("[LINUX] spawn '%s': iretq -> CPL=3 now\n", path);
    user_enter(entry_va, rsp, -1, 0);
...
/* ============================================================
 * ★ 第 19 步任务 12：最小 fork 语义
 *
 * 人工必须审查（关键安全点）：
 *   1) 只复制 PTE 存在且 PTE_USER 的页；内核页、页表页一律跳过。
 *   2) 每页必须新分配物理页并复制内容，绝不共享。
 *   3) 新 PML4 顶层先复制 PML4[0]/[256]/[511]（内核共享），
 *      然后逐个复刻 PML4[1..255] 中已存在且 PTE_USER 的子树。
 *   4) 复制失败必须完整回滚：释放已复制的新页与整个新 PML4。
 *   5) 完成后对新 PML4 调用 vmm_flush_tlb（仅当它是当前 CR3）。
 *   6) 本步不做写时复制（COW）；任何页都是 deep copy。
 * ============================================================ */
int user_fork_address_space(struct task_t *parent, struct task_t *child)
{
    if (!parent || !child) return OB_EINVAL;

    struct user_ctx *pc = user_get_ctx(parent);
    if (!pc || !pc->pml4) return OB_EPERM;

    struct user_ctx *cc = (struct user_ctx *)kzalloc(sizeof(*cc));
    if (!cc) return OB_ENOMEM;
    {
        uint8_t *dst = (uint8_t *)cc;
        const uint8_t *src = (const uint8_t *)pc;
        for (unsigned i = 0; i < sizeof(*cc); ++i) dst[i] = src[i];
    }

    uint64_t *new_pml4 = vmm_create_address_space();
    if (!new_pml4) { kfree(cc); return OB_ENOMEM; }

    for (int i4 = 1; i4 < 256; ++i4) {
        uint64_t pml4e = pc->pml4[i4];
        if (!(pml4e & PTE_PRESENT)) continue;
        if (!(pml4e & PTE_USER)) continue;

        uint64_t src_pdpt_pa = pml4e & PTE_ADDR_MASK;
        if (!vmm_is_pt_page_strict_pub(src_pdpt_pa)) continue;
        uint64_t *src_pdpt = (uint64_t *)(DIRECTMAP_BASE + src_pdpt_pa);

        /* 分配新 pdpt */
        struct page *pp = pmm_alloc_pages(0);
        if (!pp) goto rollback;
        uint64_t dst_pdpt_pa = page_to_phys(pp);
        uint64_t *dst_pdpt = (uint64_t *)(DIRECTMAP_BASE + dst_pdpt_pa);
        for (int k = 0; k < 512; ++k) dst_pdpt[k] = 0;
        new_pml4[i4] = dst_pdpt_pa
            | PTE_PRESENT | PTE_WRITABLE | PTE_USER;

        for (int i3 = 0; i3 < 512; ++i3) {
            uint64_t pdpte = src_pdpt[i3];
            if (!(pdpte & PTE_PRESENT)) continue;
            if (!(pdpte & PTE_USER)) continue;
            if (pdpte & PTE_HUGE) continue;

            uint64_t src_pd_pa = pdpte & PTE_ADDR_MASK;
            if (!vmm_is_pt_page_strict_pub(src_pd_pa)) continue;
            uint64_t *src_pd = (uint64_t *)(DIRECTMAP_BASE + src_pd_pa);

            struct page *dpp = pmm_alloc_pages(0);
            if (!dpp) goto rollback;
            uint64_t dst_pd_pa = page_to_phys(dpp);
            uint64_t *dst_pd = (uint64_t *)(DIRECTMAP_BASE + dst_pd_pa);
            for (int k = 0; k < 512; ++k) dst_pd[k] = 0;
            dst_pdpt[i3] = dst_pd_pa
                | PTE_PRESENT | PTE_WRITABLE | PTE_USER;

            for (int i2 = 0; i2 < 512; ++i2) {
                uint64_t pde = src_pd[i2];
                if (!(pde & PTE_PRESENT)) continue;
                if (!(pde & PTE_USER)) continue;
                if (pde & PTE_HUGE) continue;

                uint64_t src_pt_pa = pde & PTE_ADDR_MASK;
                if (!vmm_is_pt_page_strict_pub(src_pt_pa)) continue;
                uint64_t *src_pt = (uint64_t *)(DIRECTMAP_BASE + src_pt_pa);

                struct page *ptp = pmm_alloc_pages(0);
                if (!ptp) goto rollback;
                uint64_t dst_pt_pa = page_to_phys(ptp);
                uint64_t *dst_pt = (uint64_t *)(DIRECTMAP_BASE + dst_pt_pa);
                for (int k = 0; k < 512; ++k) dst_pt[k] = 0;
                dst_pd[i2] = dst_pt_pa
                    | PTE_PRESENT | PTE_WRITABLE | PTE_USER;

                for (int i1 = 0; i1 < 512; ++i1) {
                    uint64_t pte = src_pt[i1];
                    if (!(pte & PTE_PRESENT)) continue;
                    if (!(pte & PTE_USER)) continue;

                    uint64_t src_pa = pte & PTE_ADDR_MASK;
                    if (src_pa == 0) continue;

                    struct page *np = pmm_alloc_pages(0);
                    if (!np) goto rollback;
                    uint64_t npa = page_to_phys(np);

                    const uint8_t *s =
                        (const uint8_t *)(DIRECTMAP_BASE + src_pa);
                    uint8_t *d = (uint8_t *)(DIRECTMAP_BASE + npa);
                    for (uint64_t k = 0; k < PAGE_SIZE; ++k) d[k] = s[k];

                    uint64_t nflags = pte & ~PTE_ADDR_MASK;
                    nflags |= PTE_USER;
                    dst_pt[i1] = npa | nflags;
                }
            }
        }
    }
    ...
    serial_printf("[FORK] pid %llu -> %llu, PML4 copied\n",
                  (unsigned long long)parent->pid,
                  (unsigned long long)child->pid);
    return 0;

rollback:
    vmm_free_user_pagetables(new_pml4);
    vmm_destroy_address_space(new_pml4);
    kfree(cc);
    serial_printf("[FORK] pid %llu -> %llu, rollback\n",
                  (unsigned long long)parent->pid,
                  (unsigned long long)child->pid);
    return OB_ENOMEM;
}

extern uint64_t (*win32_get_stub_lookup(void))(const char *, const char *);
...
int user_spawn_win32_pe(const char *path)
{
    ...
    if (!t || !th) return -1;
    if (t->user_ctx) {
        serial_printf("[WIN32] spawn: pid=%llu already has user_ctx\n",
                      (unsigned long long)t->pid);
        return -1;
    }
    ...
    int rc = vfs_lookup(path, &ino);
    if (rc != 0 || !ino) {
        serial_printf("[WIN32] spawn: lookup '%s' failed rc=%d\n", path, rc);
        return -1;
    }
    uint64_t fsize = ino->size;
    if (fsize == 0 || fsize > USER_IMAGE_MAX) return -1;
    ...
    /* 2) 检测 PE */
    int d = pe_detect(img, fsize);
    if (d != 1) {
        serial_printf("[WIN32] spawn: '%s' not PE32+ (d=%d)\n", path, d);
        pmm_free_pages(img_pg, order);
        return -1;
    }

    struct pe_info info;
    rc = pe_parse_info(img, fsize, &info);
    if (rc != 0) { pmm_free_pages(img_pg, order); return rc; }
    serial_printf("[PE] detected PE32+ EXE machine=0x8664 sections=%u "
                  "entry_rva=0x%llx\n",
                  (unsigned)info.num_sections,
                  (unsigned long long)info.entry_rva);

    /* 3) 建立用户环境：compat_type 必须在 user_setup 之前设置 */
    t->compat_type = COMPAT_TYPE_WIN32;
    int urc = user_setup(t, info.image_base_pref + info.entry_rva);
    if (urc != 0) { pmm_free_pages(img_pg, order); return urc; }
    ...
    /* 4) 加载 PE 段 */
    uint64_t entry_va = 0, image_base = 0;
    uint32_t size_of_image = 0;
    rc = pe_load_into_task(t, img, fsize, &entry_va, &image_base,
                           &size_of_image);
    if (rc != 0) { pmm_free_pages(img_pg, order); return rc; }

    /* 5) PEB / TEB */
    rc = win32_setup_peb_teb(t, image_base, path);
    if (rc != 0) { pmm_free_pages(img_pg, order); return rc; }

    /* 6) stub 页 */
    rc = win32_stub_init(t, WIN32_STUB_BASE);
    if (rc != 0) { pmm_free_pages(img_pg, order); return rc; }

    /* 7) IAT 解析 */
    rc = pe_resolve_imports(t, img, fsize, image_base,
                            win32_get_stub_lookup());
    pmm_free_pages(img_pg, order);
    if (rc != 0) {
        serial_printf("[WIN32] spawn: pe_resolve_imports rc=%d\n", rc);
        return rc;
    }

    serial_printf("[WIN32] entry=0x%llx image_base=0x%llx "
                  "size=0x%x (CPL=3 next)\n",
                  (unsigned long long)entry_va,
                  (unsigned long long)image_base,
                  (unsigned)size_of_image);

    /* 8) 进入用户态；argc<0 表示栈已由 loader 布置 */
    user_enter(entry_va, c->stack_top, -1, 0);
    return 0;
}
